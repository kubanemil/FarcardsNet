...
namespace HttpFarcardService
{
	public class FarcardHttpService
	{
		...
		private Dictionary<string, ActionRequest> _delegateDictionary = new Dictionary<string, ActionRequest>();
		...
		void InitDelegateFromSettings()
		{
			...
			if (!_delegateDictionary.ContainsKey(_serverSetting.GetCardInfoEx))
			{
				_delegateDictionary.Add(_serverSetting.GetCardInfoEx, GetCardInfoEx);
				_logger.Info($"AddDelegate: {nameof(GetCardInfoEx)} _ {_serverSetting.GetCardInfoEx}");
			}

			if (!_delegateDictionary.ContainsKey(_serverSetting.TransactionsEx))
			{
				_delegateDictionary.Add(_serverSetting.TransactionsEx, TransactionsEx);
				_logger.Info($"AddDelegate: {nameof(TransactionsEx)} _ {_serverSetting.TransactionsEx}");
			}

			if (!_delegateDictionary.ContainsKey(_serverSetting.FindEmail))
			{
				_delegateDictionary.Add(_serverSetting.FindEmail, FindEmail);
				_logger.Info($"AddDelegate: {nameof(FindEmail)} _ {_serverSetting.FindEmail}");
			}

			if (!_delegateDictionary.ContainsKey(_serverSetting.GetCardImageEx))
			{
				_delegateDictionary.Add(_serverSetting.GetCardImageEx, GetCardImageEx);
				_logger.Info($"AddDelegate: {nameof(GetCardImageEx)} _ {_serverSetting.GetCardImageEx}");
			}

			_logger.Info("RouteServer");

			foreach (var delegates in _delegateDictionary)
			{
		...
		void ProcessRequest(RequestContext context)
		{
			...
			try
			{
				...
				request.ContentType.Contains("json")))
				{
					...
					if (_delegateDictionary.ContainsKey(localPath))
					{
						_logger.Info($"Invoke delegate {_delegateDictionary[localPath].Method.Name} by {localPath}");
						_delegateDictionary[localPath].Invoke(context, body);
					}
					else
					{
		...
		{
			var isXml = RequestIsXml(context);
			try
			{
		...
		private void DisposeServer()
		{
			try
			{
				if (_farcardProcessor != null)
				{
					_farcardProcessor.Dispose();
					_farcardProcessor = null;
				}
			}
			catch (Exception ex)
			{
